...
public:
    MeshTriangle(const std::string& filename)
    {
        ...
        std::cout << "Loaded mesh with " << mesh.Vertices.size() << " vertices"
                  << std::endl;
        ...
        for (auto& tri : triangles)
            ptrs.push_back(&tri);
        std::cout << "Building BVH from " << ptrs.size() << " triangles"
                  << std::endl;
...
{
    // Equation: E + tD = (1 - u - v)p0 + up1 + vp2
    // => [t, u, v]T = 1/(S1*E1) * [S2*E2, S1*S, S2*D]T
    // E1 = p1 - p0, E2 = p2 - p0, S = orig-p0, S1 = cross(dir, E2), S2 = cross(S, E1)
    Intersection inter;
